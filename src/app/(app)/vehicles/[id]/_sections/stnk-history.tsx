...
const STNK_LABELS: { [key: string]: string } = {
  tahunan: "Tahunan",
...
export function StnkHistorySection({ records, deleteAction }: Props) {
  const [deletingId, setDeletingId] = useState<string | null>(null);
  const { toast } = useToast();

  async function handleDelete(formData: FormData) {
    const id = formData.get("id") as string;
    if (!confirm("Yakin ingin menghapus data STNK ini?")) return;
    setDeletingId(id);
    const result = await deleteAction(formData);
    if (result?.error) {
      toast(result.error, "error");
    } else {
      toast("Data STNK berhasil dihapus.", "success");
    }
    setDeletingId(null);
  }
